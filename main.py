...
@app.route('/')
def home():
	return render_template('harvest.html', sitekey=config['sitekey'], domain=config['domain'])
...
if __name__ == '__main__':
	...
	with open('config.json') as file:
		...
		file.close()
	n_logging("*****************************************************")
	n_logging("Masterpeace | Masterpeacerecords")
	n_logging("*****************************************************")
	n_logging("Server running at masterpeace.{}:5000/".format(config['domain']))
	webbrowser.open('http://masterpeace.{}:5000/'.format(config['domain']))
	app.run()
